...
def weightScaleEverything(answers, weights, users, hlUsers, starts, ends, repDF):
    ...
    if len(scaledArrs) > 0:
        ...
        return scaledArrs[0], scaledArrs[1], scaledArrs[2], scaledArrs[3], scaledArrs[
...
def scoreCoding(answers, users, dfunc, scale=1, num_choices=25):
    ...
    if dfunc == 'ordinal':
        highscore, winner, weights, secondWinner, secondScore = getWinnersOrdinal(answers, num_choices=num_choices,
                                                                                  scale=scale)
    else:
